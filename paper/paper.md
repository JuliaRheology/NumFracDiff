...
---
# Summary
Fractional-order derivatives generalize the ordinary derivative to non-integer orders and are widely used in various fields of mathematics and engineering [@oldham1974fractional]. In rheology, they describe materials with long-term memory [@mainardi2022fractional]; in hydrogeology and the physics of porous media, they underpin models of anomalous transport and sub-diffusion [@su2020fractional]; they also appear in fractional-order control systems [ref: Monje, C.A., Chen, Y., Vinagre, B.M., Xue, D., Feliu-Batlle, V. (2010). Fractional-order Systems and Controls: Fundamentals and Applications.], viscoelastic structural damping, and signal processing [ref: Ortigueira, M.D. & Machado, J.A.T. (2010). "On the fractional signals and systems,"], wherever a system's present state depends on its entire past history rather than on recent values alone.
...
NumFracDiff.jl is a Julia package [@bezanson2017julia] for computing the fractional derivative of sampled signals under the three classical definitions (Grünwald–Letnikov, Riemann–Liouville, and Caputo [@oldham1974fractional; @diethelm2010analysis; @podlubny1998fractional]), implemented through several interchangeable numerical schemes. In addition to a direct single and multithreaded evaluation of the complete history, the package provides memory-truncation schemes for the Grünwald–Letnikov and Riemann–Liouville definitions based on the classical short-memory principle [@podlubny1998fractional; @li2015numerical]. These schemes include an optional analytical correction that restores much of the accuracy lost by truncating the history. For the Grünwald–Letnikov definition, the package also provides an accelerated full-history evaluation based on the FFT. 
...
**NumFracDiff** was built from the ground instead of using or contributing to other libraries for two main reasons. First, existing tools either assume analytical function inputs or solve forward differential equations, whereas several engineering and physical applications are fundamentally data-driven: NumFracDiff treats fractional differentiation strictly as an isolated, discrete array-to-array transformation, operating directly on raw sampled vectors without the overhead of wrapping data in interpolating functions. Second, because fractional derivatives are non-local operators requiring the entire past history, repeated evaluation over large datasets becomes exceptionally heavy; achieving the necessary performance required separating the one-time cost of generating convolution weights from the per-call computation via a reusable workspace, free of the compilation overhead and dependencies of more generalized packages.
...
The mathematical parameters of the operation are encapsulated in a mutable NumDiffProblem structure. This problem configuration pairs with an immutable NumDiffWorkspace structure, which serves as the pre-allocated computational backend holding the cached weights and the output derivative vectors. Decoupling the problem setup from execution provides a major performance advantage in contexts where the same signal length and time-step are differentiated repeatedly — for example, inside a parameter-fitting loop — since the heavy vector allocations live inside the reusable workspace, avoiding memory reallocation and garbage-collection overhead across thousands of sequential evaluations

To maximize computational efficiency on large datasets, NumFracDiff integrates parallelization strategies at both the loop and hardware levels. The most computationally critical inner loops are optimized through systematic vectorization, instructing the compiler to generate explicit SIMD instructions that fully exploit the registers of modern processors. For methods operating on shared-memory architectures the library leverages Julia's native multithreading parallelism model. This parallelization distributes the calculation of historical convolutions and weight summations across multiple CPU cores, drastically reducing execution times.

# Research impact statement
The computational bottleneck of evaluating non-local fractional operators on large datasets has historically limited the adoption of fractional calculus in routine, real-world data processing. By isolating fractional differentiation as a high-performance, discrete array-to-array operation, NumFracDiff.jl allows fractional calculus to be applied to large experimental or simulated datasets without requiring users to build their own numerical solvers or manage low-level memory allocation.

This benefits any field where a fractional model is repeatedly fit to sampled data or evaluated at many points: rheological parameter identification, anomalous diffusion and solute transport in porous media, computational models for biomedical applications, and fractional-order signal and image processing. Because NumFracDiff treats fractional differentiation as a standalone operation decoupled from any specific physical model, it can be dropped into an optimization or simulation pipeline in any of these domains without modification to its public interface.

As a concrete demonstration, NumFracDiff.jl has been integrated into a dedicated branch of the RHEOS.jl library [@kaplan2020rheos] for viscoelastic modelling, where fractional operators are evaluated recursively tens of thousands of times inside parameter-fitting loops. In this integration, the short-memory and FFT-based implementations gave a speedup of around 20 times over the previous method for parameter fitting, and the FFT method gave a speedup of up to 100 times for prediction tasks. As well as demonstrating its effectiveness under rigorous, data-driven workflows, this also shows how NumFracDiff.jl can be adopted within complex external frameworks without requiring changes to their public interface.
